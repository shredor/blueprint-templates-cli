...
import {
  getFolderNamesAtDirectory,
  getTemplateFileNamesAtTemplateDirectory,
} from "./getTemplateFilesAndFolders";
import { sanitizedName } from "./inputSanitizer";
import {
  replaceStringUsingTransforms,
...
export async function createFiles(
  ...
  const options = getTemplateManifestAtTemplateDirectory(
  ...
  if (options.createFilesInFolderWithPattern) {
    containerFolderName = replaceStringUsingTransforms(
      options.createFilesInFolderWithPattern,
      name,
    );
  }
  ...
  for (const fileOrFolderName of directoryContents) {
    ...
    try {
      await fs.copy(pathToContents, finalPath, {
        overwrite: false,
        errorOnExist: true,
        recursive: true,
      });
...
async function createFilesForTemplateFolder(
  ...
  const templateFilesInFolder = await getTemplateFileNamesAtTemplateDirectory(
    templateFolderPath,
  );
