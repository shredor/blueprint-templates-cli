...
export async function getFolderNamesAtDirectory(
...
export async function getTemplateFileNamesAtTemplateDirectory(
...
function shouldIgnoreFileName(fileName: string): boolean {
  ...
  return ignoredFileNames.some(ignoredFileName =>
    fileName.startsWith(ignoredFileName),
  );
}
